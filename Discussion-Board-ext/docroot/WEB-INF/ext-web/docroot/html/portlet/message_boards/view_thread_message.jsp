...
<tr>
<td class="lfr-top thread-body-width_CT_MB"" >
<div class="thread-top float-container" >
    <div class="hide-user-detail">
    <c:choose>
        ...
        <c:otherwise>
            ...
            <c:if test="<%= Validator.isNotNull(ranks[0]) %>">
                <div class="thread-user-rank" style="clear: both;">
                    <span><liferay-ui:message key="rank" />:</span> <%= ranks[0] %>
                </div>
            </c:if>

            <div class="thread-user-post-count" style="clear: both;">
                <span><liferay-ui:message key="posts" />:</span> <%= posts %>
            </div>

            <div class="thread-user-join-date" style="clear: both;">
                <span><liferay-ui:message key="join-date" />:</span>
                <%=dateFormatObj.format(message.getModifiedDate()) %>
            </div>
    ...
    <div class="subject">
        <c:choose>
            <c:when test="<%= showPermanentLink %>">
                <a href="#<portlet:namespace />message_<%= message.getMessageId() %>" title="<liferay-ui:message key="permanent-link-to-this-item" />"><strong style="font-weight: 400;font-size: 1.3em;color: #12B0E5;"><%= HtmlUtil.escape(message.getSubject()) %></strong></a>
            </c:when>
            <c:otherwise>
                <strong style="font-weight: 400;font-size: 1.3em;color: #12B0E5;"><%= HtmlUtil.escape(message.getSubject()) %></strong>
            </c:otherwise>
        </c:choose>
        ...
        <%
            User userObj=UserLocalServiceUtil.getUser(message.getUserId());
        %>

        <p class="author"><a href="./viewtopic.php?style=40&amp;p=11#p11"><i class="icon-user-thread-by"></i> </a>by <strong style="font-weight: normal;font-size: 13px;"><a href="./memberlist.php?style=40&amp;mode=viewprofile&amp;u=2" style="color: #AA0000;" class="username-coloured"><%=userObj.getFullName()%></a></strong style="font-weight: normal;"> » <%=dateFormatObj.format(message.getModifiedDate()) %> </p>
...
<td class="lfr-top user-info user-detail-hide" <%= editable ? "rowspan=\"2\"" : StringPool.BLANK %>>
    <div class="show-user-detail">
    <c:choose>
        <c:when test="<%= message.isAnonymous() %>">
            <liferay-ui:message key="anonymous" />
        </c:when>
        <c:otherwise>
            ...
            <div class="thread-user-join-date" style="clear: both;">
                <span><liferay-ui:message key="join-date" />:</span>
                 <%-- <%= dateFormatDate.format(userDisplay.getCreateDate()) %> --%>
                <%=dateFormatObj.format(message.getModifiedDate()) %>
            </div>
