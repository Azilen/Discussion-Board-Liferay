...
<%-- <%@ include file="/html/portlet/message_boards/root_configuration_getCategory.jsp" %> --%>

<%
String currentCategory=(String)session.getAttribute("MBCategoryNameParam");
String currentCategoryId=(String)session.getAttribute("MBCategoryIdParam");
...
<%
 	...
 	PortletURL rootConfigurationURL = renderResponse.createRenderURL();
	rootConfigurationURL.setParameter("struts_action","/portlet_configuration/root_configurationResponse"); //current page path
	
	if(tabs2.equals("Root Configuration"))
	{
		configurationURL=rootConfigurationURL.toString();
	}
	else
	{
	}
...
<aui:form action="<%= configurationURL %>" method="post" name="fm" onSubmit='<%= "event.preventDefault(); " + renderResponse.getNamespace() + "saveConfiguration();" %>'>
	...
	<c:choose>
		...
		<c:when test='<%= tabs2.equals("Root Configuration") %>'>
			...
			<aui:input name="windowKey" type="hidden" value="<%= windowKey %>" />
			<aui:input name="categoryId" type="hidden" value="-1" />
			<aui:input name="categoryName" type="hidden" value="home" />
		
			<aui:model-context bean="<%= category %>" model="<%= MBCategory.class %>" />
		
			<aui:fieldset>
				<aui:field-wrapper label="Root Category">
					...
					parentCategoryName=currentCategory;
					%>
					
					<div class="input-append" id="selectCategory_MB">
						<liferay-ui:input-resource id="parentCategoryName" url="<%= currentCategory %>" />
						<aui:button name="selectCategoryButton" value="select" />
						<aui:button disabled="<%= (parentCategoryId <= 0) %>" name="removeCategoryButton" onClick='<%= renderResponse.getNamespace() + "removeCategory();" %>' value="Reset" />
					</div>
				</aui:field-wrapper>
			</aui:fieldset>
		</c:when>
...
<aui:script>
	function <portlet:namespace />saveConfiguration() {
		...
		var categoryIdParam=document.<portlet:namespace />fm.<portlet:namespace />categoryId.value;
		
		var windowKeyParam=document.<portlet:namespace />fm.<portlet:namespace />windowKey.value;
		//document.<portlet:namespace />fm.action = "../../c/portlet_configuration/root_configuration?windowKey="+windowKeyParam+"&categoryId="+categoryIdParam;
		//alert(document.<portlet:namespace />fm.action);
...
<aui:script>
	function <portlet:namespace />removeCategory() {
		document.<portlet:namespace />fm.<portlet:namespace />categoryId.value = "-1";

		
		document.getElementById('<portlet:namespace />parentCategoryName').value = "<%=currentCategory %>";
			...
			Liferay.Util.selectEntity(
				{
					...
					uri: '<portlet:renderURL windowState="<%= LiferayWindowState.POP_UP.toString() %>"><portlet:param name="struts_action" value="/message_boards/select_category" /><portlet:param name="mbCategoryId" value="<%= currentCategoryId %>" /></portlet:renderURL>'
				},
				function(event) {
					...
					document.<portlet:namespace />fm.<portlet:namespace />categoryName.value = event.name;
					document.getElementById('<portlet:namespace />parentCategoryName').value = event.name;

					document.<portlet:namespace />fm.action = "<%=rootConfigurationURL.toString() %>"+"&windowKey=1234";
