...
//Customize code for discussion board
long preferenceCatId=0;
long groupId=0;

if(themeDisplay.getPortletDisplay().getPortletName().equalsIgnoreCase(MBPortletKeys.MESSAGE_BOARDS)){
	request.getSession().setAttribute("categoryId", String.valueOf(categoryId));
	
	MBCategory selectedCategory=(MBCategory)session.getAttribute("selectedCategory");
	...
	PortletPreferences preference=(PortletPreferences)request.getAttribute("preference");
	 preferenceCatId=Long.parseLong(preference.getValue("categoryID", "-1"));
	 groupId=GetterUtil.getLong(request.getAttribute("groupId"));
}
...
<div class="container-fluid-1280 view-entries-container">
	<c:if test="<%= category != null %>">
		...
		<%
		//Customize code to hide back button from portlet.
	...
	<aui:form action="<%= portletURL.toString() %>" method="get" name="fm">
		...
		<c:choose>
		<c:when test="<%=themeDisplay.getPortletDisplay().getPortletName().equalsIgnoreCase(MBPortletKeys.MESSAGE_BOARDS)%>">
			<%
		...
		%>
			...
			<liferay-ui:search-container-results>
						<%
						Object resultsList=request.getAttribute("resultsList");
						String pageType= (String)request.getAttribute("pageType");
							if(resultsList != null){
									if(pageType.equalsIgnoreCase("Posts")){
										 List<MBThread> threadResults =(List<MBThread>) resultsList;
										 if(threadResults != null){
											List<MBThread> resultsThread = ListUtil.subList(threadResults,searchContainer.getStart(),searchContainer.getEnd());
											int size = threadResults.size();
											pageContext.setAttribute("results", resultsThread);
										  ...
										  List<MBThread> threadResults =(List<MBThread> ) request.getAttribute("resultsList");
										 if(threadResults != null){
											List<MBThread> resultsThread = ListUtil.subList(threadResults,searchContainer.getStart(),searchContainer.getEnd());
											int size = threadResults.size();
											pageContext.setAttribute("results", resultsThread);
										  ...
										  List<MBCategory> categoryResults=(List<MBCategory>)resultsList;
										List<MBCategory> resultsCategory = ListUtil.subList(categoryResults,searchContainer.getStart(),searchContainer.getEnd());
										int size = categoryResults.size();
										pageContext.setAttribute("results", resultsCategory);
										pageContext.setAttribute("total", size);
									}
							}
							else{
								searchContainer.setEmptyResultsMessage((String)request.getAttribute("emptyMessage"));
			...
			<liferay-ui:search-iterator displayStyle='<%= GetterUtil.getString(request.getAttribute("view.jsp-displayStyle")) %>' markupView="lexicon" resultRowSplitter="<%= new MBResultRowSplitter() %>"  paginate="<%=true%>" />
		</liferay-ui:search-container>
		</c:when>
		<c:otherwise>
				<liferay-ui:search-container
